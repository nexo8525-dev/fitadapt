...
import { Check, ChevronDown, ChevronUp, Clock3, Dumbbell, Flame, Loader2, LogOut, RefreshCw, Sparkles, Target, Utensils, X, Zap, CalendarDays, CheckCircle2, Moon, ListChecks, Play, TrendingUp, Settings } from 'lucide-react';
...
interface ActivityMap {
  [day: string]: { completed: boolean; tracking_data?: any; };
}
...
function buildActivityMap(rows: any[]): ActivityMap {
  ...
  for (const row of rows) {
    if (typeof row?.day === 'string') {
      map[row.day] = { completed: Boolean(row.completed), tracking_data: row.tracking_data || {} };
    }
  }
  return map;
}
...
export default function DashboardPage() {
  ...
  const [activityError, setActivityError] = useState<string>(''); // Feature 13 Error State
  ...
  const finishWorkout = async () => {
    ...
    try {
      ...
      setActivityError(err.message || 'Failed to save workout data. Check connection and try again.');
    } finally {
  ...
  const saveDietMeal = async () => {
    ...
    try {
      ...
      setActivityError(err.message || 'Failed to save diet data. Check connection and try again.');
    } finally {
      setActivityLoading(null);
    }
  };
  ...
  if (dashboard && dashboard.hasProfile && !dashboard.workout && !dashboard.diet) {
    return (
      <div className="min-h-screen bg-slate-950 p-6 flex flex-col items-center justify-center">
        ...
        <button onClick={generateFirstPlan} disabled={generatingPlan} className="bg-emerald-600 px-7 py-3 rounded-xl text-white font-bold flex items-center gap-2 disabled:opacity-50 transition-all">
      ...
      <header className="border-b border-slate-800 bg-slate-950/90 pt-8 pb-6 px-6 md:px-8">
        <div className="mx-auto max-w-5xl flex items-start md:items-end justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-white">Good morning, {user?.firstName || 'User'}</h1>
          </div>
          <div className="flex items-center gap-2 md:gap-3">
            <button onClick={() => window.location.href = '/settings'} className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-900 p-2.5 md:px-4 md:py-2.5 text-slate-400 hover:bg-slate-800 hover:text-white transition-all"><Settings className="h-5 w-5" /><span className="hidden md:inline text-sm font-semibold">Settings</span></button>
            <button onClick={handleLogout} className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-900 p-2.5 md:px-4 md:py-2.5 text-slate-400 hover:bg-red-950/30 hover:text-red-400 hover:border-red-900/50 transition-all"><LogOut className="h-5 w-5" /><span className="hidden md:inline text-sm font-semibold">Logout</span></button>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl px-6 py-6 md:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          ...
          <div className="flex w-full md:w-auto gap-3">
            <button onClick={() => window.location.href = '/plan'} className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors shadow-lg shadow-indigo-600/20"><ListChecks className="w-4 h-4" /> View Plan</button>
            <button onClick={() => window.location.href = '/progress'} className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-slate-900 border border-slate-800 hover:bg-slate-800 text-slate-300 px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors"><TrendingUp className="w-4 h-4" /> Progress</button>
          </div>
        </div>
        ...
        {viewMode === 'week' && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center text-slate-400">
              ...
              <p className="max-w-sm mx-auto">Switch to "Today" to log your daily tasks. Use "View Plan" or "Progress" to see full details.</p>
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/90 p-4 md:p-10 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl overflow-hidden my-auto">
            ...
            <div className="p-5 border-t border-slate-800 bg-slate-950">
              ...
              <button onClick={finishWorkout} disabled={activityLoading !== null} className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold py-4 rounded-xl transition-colors">
      ...
      {dietModalOpen && activeDietMeal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl overflow-hidden">
            ...
            <div className="p-5 border-t border-slate-800 bg-slate-950">
              {activityError && <div className="mb-4 bg-red-950/30 border border-red-900/50 p-3 rounded-xl text-sm font-semibold text-red-400">{activityError}</div>}
              <button onClick={saveDietMeal} disabled={activityLoading !== null} className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-bold py-3.5 rounded-xl transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/70 p-4 backdrop-blur-sm">
          <div className="my-8 w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between border-b border-slate-800 p-6 bg-slate-950">
              <div><h2 className="text-xl font-bold text-white">Weekly Check-In</h2><p className="mt-1 text-sm text-slate-500">Week {currentWeek} review</p></div>
